...
6. One applied checkpoint (Sections 2-8) that asks the reader to inspect,
   predict, modify, or diagnose the shared capstone -- not recall a
   definition -- kept under half a page, with its answer key in
   `companion/exercises/README.md` rather than next to the question.
7. A checklist that lets the reader test their understanding and design.
...
| Applied checkpoint | Inspect, predict, modify, or diagnose the shared capstone | `practice` or `checklist` callout, answer key in `companion/exercises/README.md` |
...
**Key nodes and labels:** six sequential stages carry the main flow --
`Sources`, `Ingestion`, `Storage`, `Processing and Transformation`,
`Orchestration`, and `Serving`. `Quality and Reliability`, `Metadata and
Governance`, `Observability`, `Security`, and `Cost` are cross-cutting
concerns that apply across every stage, not steps that happen after
Orchestration; draw them once, as a single shaded/dashed band spanning the
whole chain with its own label, never as additional sequential nodes.
...
The alt text must name all six sequential stages in reading order and state
that quality, governance, observability, security, and cost span them rather
than following as a seventh or eighth step. Do not encode meaning by color
alone.
...
- A data-contract figure only if the contract's producer, schema, quality
  guarantees, and consumer relationship cannot be explained in a table. This
  guide's own contract figure was retired in favor of a compact table mapped
  to the field names in its adjacent contract listing, once the figure was
  judged to restate the listing rather than teach a relationship the listing
  couldn't already show -- treat that as the default outcome unless a
  contract's producer/consumer relationship genuinely needs a diagram.
...
Use one consistent edge/region vocabulary across every diagram rather than
choosing styling per fragment. Use ReportKit's existing semantic edge kinds
(`flow`, `handoff`, `dependency`, `causal`, `optional`, `sequence` -- see
`\RKEdge`/`\flowedge`/`\networkedge` in the ReportKit engine's
`reportkit-diagrams.sty`/`reportkit-process.sty`) rather than inventing new
TikZ styles:

- **Solid path (`flow`/`dependency`):** data movement -- the normal,
  ordinary-case path a record takes.
- **Dashed path (`handoff` or `optional`):** control/scheduling relationships,
  optional branches, and exception paths (a failing check, a quarantine
  route, a blocked publish). Pair a dashed edge with a short text label
  (`fail`, `quarantine`, `documents`) so the distinction survives grayscale
  and doesn't rely on the dash pattern alone.
- **Shaded/dashed background band:** ownership or a cross-cutting concern
  that spans several nodes or the whole diagram (for example, the Section 1
  lifecycle figure's quality/governance/observability/security/cost band) --
  never draw a cross-cutting concern as an ordinary sequential node.
- **Trust boundary / gate node:** a node a reader must pass through before
  reaching a consumer (for example, the Section 9 capstone architecture's
  `contract` node) should sit in the data path's own lane sequence, not be
  reachable only by an edge that crosses another lane's node text.

Apply this vocabulary consistently; do not restyle it independently per
fragment.
